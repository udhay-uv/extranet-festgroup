...
const StatusDialog = ({ status, onClose }: { status: StatusMessage; onClose: () => void }) => (
  <Dialog open={true} onOpenChange={onClose}>
    <DialogContent className={`${status.type === 'success' ? 'bg-gradient-to-br from-green-50 to-white border-green-200' : 'bg-gradient-to-br from-red-50 to-white border-red-200'} shadow-xl`}>
      <motion.div
      ...
      >
        ...
        <h2 className={`text-2xl font-bold mb-2 ${status.type === 'success' ? 'text-green-700' : 'text-red-700'}`}>
          {status.type === 'success' ? 'Success!' : 'Error'}
        </h2>
        <p className={`text-lg ${status.type === 'success' ? 'text-green-600' : 'text-red-600'}`}>
          {status.message}
        </p>
        <Button
          onClick={onClose}
          className={`mt-6 ${status.type === 'success' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}
        >
          Close
        </Button>
      </motion.div>
...
const AddressForm = ({ 
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid gap-2">
        <Label htmlFor="billingAddress" className="text-green-700">Billing Address</Label>
        <Input
          id="billingAddress"
      ...
      <div className="grid gap-2">
        <Label htmlFor="shippingAddress" className="text-green-700">Shipping Address</Label>
        <Input
          id="shippingAddress"
      ...
      <div className="grid gap-2">
        <Label htmlFor="contactName" className="text-green-700">Contact Name</Label>
        <Input
          id="contactName"
...
export const SemiconHome = () => {
  ...
  async function getUserInfo(){
    ...
    const res = await axios.post(`${BACKEND_URL}/api/s/userinfo`,{
        company:'S',
  ...
  const handleAddAddress = async (newAddress: Address) => {
    try {
      ...
      const res = await axios.post(`${BACKEND_URL}/api/s/addaddress`,{
        ...newAddress,
        token:localStorage.getItem("s_token") || "",
        company:"S",
      });
  ...
  const handleUpdateAddress = async (updatedAddress: Address) => {
    try {
      ...
      const res = await axios.post(`${BACKEND_URL}/api/s/updateaddress`,{
        ...updatedAddress,
        token:localStorage.getItem("s_token") || "",
        company:"S",
      });
      ...
      >
        <div className="bg-white rounded-lg shadow-lg p-6 border border-green-100">
          <motion.div 
            ...
            className="mb-6"
          >
            <h2 className="text-2xl font-semibold text-green-600">User Information</h2>
          ...
          <motion.div 
            ...
            animate={{ x: 0 }}
          >
            <h3 className="text-xl font-semibold text-green-600 mb-4">Company Details</h3>
            <table className="w-full">
              <thead className="bg-green-50">
                <tr>
                  <th className="px-4 py-2 text-left text-green-600">GST No.</th>
                  <th className="px-4 py-2 text-left text-green-600">Name</th>
                  {/* <th className="px-4 py-2 text-left text-green-600">Company</th> */}
                  <th className="px-4 py-2 text-left text-green-600">Sales Person</th>
                </tr>
              </thead>
              <tbody>
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-green-600">Address Types</h3>
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="bg-gradient-to-br from-green-50 to-white border-green-200">
                  <DialogHeader>
                    <DialogTitle className="text-2xl font-bold text-green-700">Add New Address</DialogTitle>
                  </DialogHeader>
                  <AddressForm
              ...
              {address?.map((type, index) => (
                ...
                >
                  <button
                    ...
                    className="w-full bg-white border border-green-200 rounded-lg p-4 flex items-center justify-between hover:bg-green-50 transition-colors"
                  >
                          ...
                          .map((address, addressIndex) => (
                            <motion.div
                              ...
                              transition={{ delay: 0.1 }}
                                className="mt-4 bg-gradient-to-br from-green-50 to-white rounded-lg p-6 border border-green-100 shadow-md"
                            >
                              <div className="flex justify-between items-start">
                                ...
                                <Dialog>
                                  ...
                                  <DialogContent className="bg-gradient-to-br from-green-50 to-white border-green-200">
                                    <DialogHeader>
                                      <DialogTitle className="text-2xl font-bold text-green-700">Update Address</DialogTitle>
                                    </DialogHeader>
                                    <AddressForm 
